...
class SendEmail():
    def send_email(self, new_report):
        ...
        with open(new_report, 'r', encoding='utf8') as f:
            mail_body = f.read()
        # 发件人地址
        send_addr = email.send_user
        # 收件人地址
        reciver_addr = email.to_user
        # 发送邮箱的服务器地址 qq邮箱是'smtp.qq.com', 136邮箱是'smtp.136.com'
        mail_server = email.mail_server
        now = time.strftime("%Y-%m-%d %H_%M_%S")
        # 邮件标题
        subject = email.subject + now
        # 发件人的邮箱及邮箱授权码
        username = email.send_user
        password = email.password  # 注意这里是邮箱的授权码而不是邮箱密码
        # 邮箱的内容和标题
